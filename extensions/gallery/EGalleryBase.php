...
class EGalleryBase extends CWidget {
	...
	/**
	 * @var boolean album folders that are in a format parseable by {@link http://php.net/strtotime strtotime} should be displayed as a formated date
	 */
    public $displayFoldersAsDates = true;

	/**
	 * @var string the {@link http://php.net/date date format} used for outputting
	 */
	...
	protected function setup(){
		...
		if($this->path != '' && file_exists($this->_realpath)):
			...
			throw new CException('Path to gallery is not specified or invalid. Perhaps you specified an absolute URL when you meant to use a relative one.');
		endif;
	}
	 ...
	 * Otherwise it tries to parse the directory name as a date (if appropriate).
	 *
				...
				if ($image->isFile() && $image->isReadable() && preg_match("/\.(jpe?g|gif|png)$/i",$image->getFilename())) {
					...
					if(in_array($mime,$this->mimeTypes))
					{
						...
						else
						{
							...
							{
								if(is_writable($_realpath))
								{
									$_needsThumbs[] = array('folder'=>$_realpath.DIRECTORY_SEPARATOR.'thumbs','image'=>$image->getFilename());
								}
								else
								{
									Yii::log('Tried to create thumbnails but the directory was not writable: '.$_realpath, 'warning', 'app.extensions.EGallery');
									throw new CException('Tried to create thumbnails but the directory was not writable. Either configure your folder permissions or configure EGallery with \'createThumbnails\' => false,');
								}
							}
						}
...
//			$WshShell->Run("php.exe $Command", 0, false);
	...
	/**
	 * @return string the dynamic CSS file path needed for {@see EGallery} and {@see EGalleryManager}
	 */
 ...
 * This file is automatically generated, please do not edit this file directly.
 You should edit the associating PHP file instead.
 */
...
';
		...
		if($contents != file_get_contents($folder.DIRECTORY_SEPARATOR.$file.'.css'))
		{
			if(is_writable($folder.DIRECTORY_SEPARATOR.$file.'.css'))
			{
				file_put_contents($folder.DIRECTORY_SEPARATOR.$file.'.css', $contents);
			}
			else
			{
				Yii::log('Tried to generate CSS file but the directory was not writeable: '.$folder, 'warning', 'app.extensions.EGallery');
			}
		}
